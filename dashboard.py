...
st.write("Di sini kita bisa melihat distribusi pelanggan berdasarkan kota dan negara bagian menggunakan pie chart.")
...
with col3:
    ...
    fig, ax = plt.subplots()
    ax.pie(top_5_cities, labels=top_5_cities.index, autopct='%1.1f%%', startangle=90, colors=['lightblue', 'lightgreen', 'lightcoral', 'lightpink', 'lightyellow'])
...
with col4:
    ...
    fig, ax = plt.subplots()
    ax.pie(top_5_states, labels=top_5_states.index, autopct='%1.1f%%', startangle=90, colors=['lightblue', 'lightgreen', 'lightcoral', 'lightpink', 'lightyellow'])
    ax.axis('equal')
    st.pyplot(fig)
